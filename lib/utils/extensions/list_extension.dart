...
extension KtX2<T> on KtIterable<T> {
  KtList<T> plusAndMapIfAbsent<K>(
    ...
    final otherFiltered = other.filter((it) => condition(it)).distinctBy(distinctBy);
...
extension IterableX<T extends BaseEntity> on Iterable<T> {
  ...
  List<T> minusIfPresent(Iterable<T> other) => toList()..removeWhere((el) => other.any((it) => it.id == el.id));
